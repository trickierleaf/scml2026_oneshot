...
class SimpleBayesianAgent(SyncRandomOneShotAgent):
    """SyncRandomOneShotAgent を直接継承する自己完結エージェント。

    旧 BayesianAgent (ベイズ Greedy/NonGreedy 分類・探索・成功率学習) のコードを
    取り込み、オファー生成と受諾のみ単純規則で構成する。外部モジュールへの依存は
    negmas / scml のみ (このファイル単体で動作する)。
    """
