...
class BayesianAgent024(SyncRandomOneShotAgent):
    """
    ...
    def _greedy_only_first_proposals(self, needs: int, partners: list[str]):
        ...
        if greedy_partners:
            if len(greedy_partners) >= 2:
                ...
                greedy_quantity = self._single_greedy_quantity(needs, len(partners))
            ...
            if len(greedy_partners) >= 3:
                remainder_quantity = max(0, int(needs) - sum(greedy_quantities))
                if remainder_quantity > 0:
                    remainder_greedy_partner = greedy_partners[2]
                    proposals[remainder_greedy_partner] = self._offer(
                        remainder_greedy_partner,
                        remainder_quantity,
                        self._worst_price_for_me(remainder_greedy_partner),
                    )
                scaled_target = 0
        else:
            scaled_target = self._success_adjusted_quantity(needs, success_rate)

        if len(greedy_partners) >= 3 and scaled_target > 0:
    ...
    def _single_greedy_quantity(self, needs: int, cc: int) -> int:
        needs = int(needs)
        if int(cc) <= 5:
            return min(8, needs)
        return min(7, needs)
            ...
            if seller_greedy_fill is not None:
                ...
                for partner in side_partners:
                    if partner in accepted_partners:
                        ...
                        quantity = self._first_counter_quantity(
                            partner,
                            remaining_needs,
                        )
            ...
            if special_acceptance is not None:
                ...
                for partner in remaining_partners:
                    ...
                    if quantity <= 0:
                        responses[partner] = self._unneeded_response()
                        continue
                    quantity = self._first_counter_quantity(partner, quantity)
    ...
    def _first_counter_quantity(self, partner, quantity: int) -> int:
        quantity = int(quantity)
        if quantity <= 0:
            return quantity
        if self._has_sent_counter_this_step(partner):
            return quantity
        return self._clamp_quantity(partner, math.ceil(quantity * 1.2))

    def _has_sent_counter_this_step(self, partner) -> bool:
        return any(
            int(item.get("step", -1)) == int(self.awi.current_step)
            and not bool(item.get("initial", True))
            for item in self._sent_offer_history.get(partner, [])
        )
    ...
    def _type_based_first_proposals(self, needs: int, partners: list[str], base_proposals):
        ...
        if greedy_partners:
            ...
            if len(selected_greedy) == 1:
                quantities = [self._single_greedy_quantity(needs, len(partners))]
            else:
