...
1. \[recommended\] Analyze the latest tournament result. This writes a single
   interactive `report.html` useful for improving the agent into the
   tournament's `analysis` folder.

   > python -m myagent.helpers.analyzer

   You can also analyze a specific tournament stage.

   > python -m myagent.helpers.analyzer tmp_tournament_test/your-stage-folder

   See `RESULT_ANALYSIS_JA.md` for a Japanese explanation of the command and
   report.
